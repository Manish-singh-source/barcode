...
@section('content')
    @php
        ...
        $barcodeImageUrl = $barcode->barcode_image_path
            ? \Illuminate\Support\Facades\Storage::disk('public')->url($barcode->barcode_image_path)
            : null;
            ...
            <div class="card border-0 shadow-sm rounded-4 mb-4">
                <div class="card-body p-4 p-xl-5 text-center">
                    <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-4">
                        ...
                        <span
                            class="badge text-bg-{{ $barcode->is_active ? 'success' : 'secondary' }}">{{ $barcode->is_active ? 'Active' : 'Inactive' }}</span>
                    </div>

                    <div class="text-center mb-4">
                        @if ($barcodeImageUrl)
                            <img src="{{ $barcodeImageUrl }}" alt="Barcode preview"
                                class="img-fluid rounded-4 border bg-white p-3 shadow-sm" style="max-width: 400px;">
                        @else
                            <div class="border rounded-4 bg-light p-5 text-secondary">No preview available</div>
                        @endif
                    </div>

                    <div class="fw-semibold fs-5 mb-4 font-monospace">
                        {{ \App\Models\BarcodeGeneration::normalizeText($barcode->custom_label ?: $barcode->unique_code) }}
                    </div>
                    ...
                    <div class="d-flex flex-wrap justify-content-center gap-2">
                        ...
                        <button type="button" id="printBarcodeBtn" class="btn btn-outline-primary">Print Barcode</button>
                    </div>
                </div>
            </div>
                        ...
                        <table id="barcodeDetailsTable" class="table table-striped table-hover align-middle mb-0 w-100">
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td><span
                                            class="badge text-bg-dark font-monospace fs-6 px-3 py-2">{{ $barcode->unique_code }}</span>
                                    </td>
                                </tr>
                                <tr>
                                ...
                                <tr>
                                    ...
                                    <td><span
                                            class="text-break">{{ \App\Models\BarcodeGeneration::normalizeText($barcode->barcode_data) }}</span>
                                    </td>
                                </tr>
                                <tr>
        ...
        <div class="col-12 col-lg-5">
            <div class="card border-0 shadow-sm rounded-4 mb-4">
                <div class="card-body p-4 p-xl-5">
                    ...
                    <div class="fw-bold fs-5 mb-2">
                        {{ \App\Models\BarcodeGeneration::normalizeText($snapshot['name'] ?? $barcode->barcode_data) }}
                    </div>
                    <div class="text-secondary">{{ $snapshot['description'] ?: 'No structured description available.' }}
                    </div>
                    <div class="mt-3 small text-secondary">
                        @if (!empty($snapshot['sku']))
                            SKU: {{ $snapshot['sku'] }}
                        @endif
                        @if (!empty($snapshot['brand']))
                            {{ !empty($snapshot['sku']) ? ' | ' : '' }}Brand: {{ $snapshot['brand'] }}
                        @endif
                        @if (!empty($snapshot['category']))
                            {{ !empty($snapshot['sku']) || !empty($snapshot['brand']) ? ' | ' : '' }}Category:
                            {{ $snapshot['category'] }}
                        @endif
                        @if (!empty($snapshot['unit']))
                            {{ !empty($snapshot['sku']) || !empty($snapshot['brand']) || !empty($snapshot['category']) ? ' | ' : '' }}Unit:
                            {{ $snapshot['unit'] }}
                        @endif
                        @if (!is_null($snapshot['stock_quantity']))
                            {{ !empty($snapshot['sku']) || !empty($snapshot['brand']) || !empty($snapshot['category']) || !empty($snapshot['unit']) ? ' | ' : '' }}Stock:
            ...
            <div class="card border-0 shadow-sm rounded-4 mb-4">
                <div class="card-body p-4 p-xl-5">
                    ...
                    <div>
                        {{ $barcode->scan_logs_max_created_at ? \Illuminate\Support\Carbon::parse($barcode->scan_logs_max_created_at)->format('Y-m-d H:i') : 'Never scanned' }}
                    </div>
                </div>
            </div>
                ...
                <form id="editBarcodeForm" method="POST" action="{{ url('/barcodes/' . $barcode->id) }}">
                    ...
                    <div class="modal-body vstack gap-3">
                        <div>
                            ...
                            <textarea name="barcode_data" id="editBarcodeData" class="form-control" rows="4"
                                placeholder="Update barcode data">{{ \App\Models\BarcodeGeneration::normalizeText($barcode->barcode_data) }}</textarea>
                        </div>
                    </div>
...
@push('scripts')
    <script>
        (function() {
            ...
            function openPrintWindow(imageSrc, label) {
                ...
                const html = `
                    ...
                    <body>
                        <div class="barcode-print">
                            <img src="${imageSrc}" alt="Barcode print preview">
            ...
            if (printBarcodeBtn) {
                printBarcodeBtn.addEventListener('click', function() {
                    if (barcodeImageUrl) {
                        openPrintWindow(barcodeImageUrl, uniqueCode);
                        return;
                    }
                    ...
                    if (barcodeSvg) {
                        ...
                        const url = URL.createObjectURL(blob);
                        openPrintWindow(url, uniqueCode);
                        window.setTimeout(() => URL.revokeObjectURL(url), 1000);
                    }
                });
            }
